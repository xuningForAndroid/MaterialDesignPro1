...
public class ChartView extends View {
    ...
    //        private int xylinecolor = 0xffffffff;
    //xy坐标轴宽度
    private int xylinewidth = dpToPx(1);
    //xy坐标轴文字颜色
    private int xytextcolor = 0xff7e7e7e;
    //低等位线颜色
    private int lowLevelColor = 0xFFFCAC10;
    //高等位线颜色
    private int highLevelColor = 0xFFF05522;
    //xy坐标轴文字大小
...
//    private int lowLevel;//低等位线
//    private int highLevel;//高等位线
...
//        super.onDraw(canvas);
    ...
    private void drawBrokenPoint(Canvas canvas) {
        ...
        for (int i = 0; i < xValue.size(); i++) {
            ...
            Log.i(TAG, "drawBrokenPoint: "+xInit+"-----"+interval+"------"+yOri+"---xValue:---"+xValue+"--yValue--"+yValue);
            //绘制选中的点
            if (i == selectIndex - 1) {
                linePaint.setStyle(Paint.Style.FILL);
...
//                canvas.drawCircle(x, y, dp4, linePaint);
                drawFloatTextBox2(canvas, x, y - dp7, value.get(xValue.get(i)));
            }
            //绘制普通的节点
//            linePaint.setStyle(Paint.Style.FILL);
//            linePaint.setColor(Color.WHITE);
...
//        linePaint.setColor(Color.WHITE);
    ...
    private void drawFloatTextBox2(Canvas canvas, float x, float y, int text) {
        ...
        path.lineTo(x - dp6, y - dp6);
        //p3
        path.lineTo(x - dp60, y - dp6);
        //p4
        path.lineTo(x - dp60, y - dp6 - dp60);
        //p5
        path.lineTo(x + dp60, y - dp6 - dp60);
        //p6
        path.lineTo(x + dp60, y - dp6);
...
//        linePaint.setColor(Color.WHITE);
        linePaint.setColor(linecolor);
        linePaint.setTextSize(spToPx(14));
        Rect rect = getTextBounds(text + "mmol/L", linePaint);
        canvas.drawText(text + "mmol/L", x - rect.width() / 2, y - dp6 - (dp70 - rect.height()) / 2, linePaint);
        Rect rect2 = getTextBounds("05月13日-早餐", linePaint);
        canvas.drawText("05月13日-早餐", x - rect2.width() / 2, y - dp6 - (dp35 - rect2.height()) / 2, linePaint);
    }
    ...
    private void drawBrokenLine2(Canvas canvas) {
        ...
        for (int i = 1; i < xValue.size(); i++) {
            path = new Path();
            path.moveTo(x, y);
            Integer color = mLinesColors.get(i - 1);
...
//        canvas.drawPath(path, linePaint);
//    }
    List<Integer> mLinesColors;

    public void setLinesColors(List<Integer> colors) {
        this.mLinesColors = colors;
    }
    ...
    private void drawXY(Canvas canvas) {
        int length = dpToPx(width);//刻度的长度
        int dp40 = dpToPx(30);
        //绘制Y坐标
        Log.i(TAG, "drawXY: " + xOri + "-------" + yOri);
        //绘制y坐标值
...
//        canvas.drawPath(path, xyPaint);
        ...
        for (int i = 0; i < yValue.size(); i++) {
            Integer currentValue = yValue.get(i);
            //绘制Y轴刻度
            if (currentValue == lowLevel) {//低等位线
                xyPaint.setPathEffect(new DashPathEffect(new float[]{2, 8}, 0));
                xyPaint.setColor(lowLevelColor);
                canvas.drawLine(xOri, yOri - yLength * i + xylinewidth / 2 - dp40, xOri + width - 120, yOri - yLength * i + xylinewidth / 2 - dp40, xyPaint);
                //绘制Y轴文本
...
//            canvas.drawText(text, 0, text.length(), xOri - xylinewidth - dpToPx(2) - rect.width(), yOri - yLength * i + rect.height() / 2, xyTextPaint);
                //y轴文本绘制在右侧
                canvas.drawText(text, 0, text.length(), width - rect.width() - dpToPx(5), yOri - yLength * i + rect.height() / 2+100, xyTextPaint);
            } else if (currentValue == highLevel) {//高等位线
                xyPaint.setColor(highLevelColor);
                xyPaint.setPathEffect(new DashPathEffect(new float[]{2, 8}, 0));
                canvas.drawLine(xOri, yOri - yLength * i + xylinewidth / 2 - dp40, xOri + width - 120, yOri - yLength * i + xylinewidth / 2 - dp40, xyPaint);
...
//            canvas.drawText(text, 0, text.length(), xOri - xylinewidth - dpToPx(2) - rect.width(), yOri - yLength * i + rect.height() / 2, xyTextPaint);
                ...
                xyPaint.setColor(xylinecolor);
                canvas.drawLine(xOri, yOri - yLength * i + xylinewidth / 2, xOri + width - 120, yOri - yLength * i + xylinewidth / 2, xyPaint);
...
//            canvas.drawText(text, 0, text.length(), xOri - xylinewidth - dpToPx(2) - rect.width(), yOri - yLength * i + rect.height() / 2, xyTextPaint);
                //y轴文本绘制在右侧
                canvas.drawText(text, 0, text.length(), width - rect.width() - dpToPx(5), yOri - yLength * i + rect.height() / 2, xyTextPaint);
            }
...
//        }
     ...
     * @param highLevel
     */
    public void setDivideValue(int lowLevel, int highLevel) {
        this.lowLevel = lowLevel;
        this.highLevel = highLevel;
        int lowPos = getLowPos(lowLevel);
        yValue.add(lowPos, lowLevel);
        int highPos = getLowPos(highLevel);
        yValue.add(highPos, highLevel);

    }

    private int getLowPos(int lowLevel) {
        int pos = 0;
        for (int i = 0; i < yValue.size() - 1; i++) {
            Integer pre = yValue.get(i);
            Integer next = yValue.get(i + 1);
            if (lowLevel > pre && lowLevel < next) {
                pos = i + 1;
            }
        }
        Log.i(TAG, "getLowPos: " + pos);
        return pos;
    }
}
